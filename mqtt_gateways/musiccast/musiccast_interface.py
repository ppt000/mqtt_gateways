...
class musiccastInterface(object):
    ...
    Args:
        params (dictionary): includes whatever options were in the dedicated section
            of the configuration file.  This class only requires the *sysdefpath* option
            to be defined as the location of the JSON file describing the system.  If
            that option is not found the local directory is used instead.
    '''
    ...
    def __init__(self, params, msglist_in, msglist_out):
        ...
        self._system = mcs.System(json_data, self._msgl_out)
        # now all the static data is loaded in the structure, but maybe not the 'live' data
    ...
    def loop(self):
        ...
        by the incoming MQTT message: if it is clearly addressed specifically to this
        ...
        is executed, otherwise it stays silent as the message is probably intended to
        somebody else.
        '''

        # process the incoming messages list
        while True:
            # read messages on a FIFO basis
